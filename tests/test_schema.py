...
class TestFieldTypes:
    ...
    def test_short_text_field(self):
        ...
        assert 0.93 <= results[0].score <= 1.0, f"Expected score in [0.93, 1.0] for 'Jon Doe' vs 'John Doe' (Jaro-Winkler), got {results[0].score}"
    ...
    def test_long_text_field(self):
        ...
        assert all("fox" in desc.lower() or "brown" in desc.lower() for desc in top_two_descriptions), \
    ...
    def test_token_set_field(self):
        ...
        assert 0.6 <= results[0].score <= 0.75, f"Expected score in [0.6, 0.75] for 2/3 tag overlap, got {results[0].score}"
...
class TestMultiFieldSearch:
    ...
    def test_multi_field_query(self):
        ...
        assert 0.75 <= results[0].score <= 0.95, f"Expected score in [0.75, 0.95] for multi-field match with strong name and tag overlap, got {results[0].score}"
...
class TestPerformance:
    ...
    def test_repeated_searches(self):
        ...
        assert results2[0].id == 42, f"Expected item_42 as best match on second search, got id {results2[0].id}"
        ...
        assert results1[0].score == 1.0, f"Expected score 1.0 for exact match 'item_42', got {results1[0].score}"
        assert results2[0].score == 1.0, f"Expected score 1.0 for exact match on second search, got {results2[0].score}"
